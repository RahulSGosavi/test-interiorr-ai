...
# Pricing AI Service
from pricing_ai_service import process_question
...
async def pricing_ai_query(
    ...
    # Process question using new pricing AI service
    try:
        original_filename = cast(str, db_file.name) if db_file.name else file_path.name
        result = await process_question(file_path, query.question, query.provider, original_filename)
        return AIResponse(
            response=result.get("response", "No response"),
            table=result.get("table"),
            provider=result.get("provider", query.provider),
        )
    except Exception as e:
        logger.error(f"Pricing AI error: {e}", exc_info=True)
        return AIResponse(
