...
const webpackConfig = {
  webpack: {
    ...
    configure: (webpackConfig) => {
      ...
      // Exclude pdfjs-dist/react-pdf from source-map-loader to avoid missing source errors
      if (webpackConfig.module && Array.isArray(webpackConfig.module.rules)) {
        webpackConfig.module.rules.forEach((rule) => {
          if (rule.use) {
            const uses = Array.isArray(rule.use) ? rule.use : [rule.use];
            const hasSourceMapLoader = uses.some((loader) =>
              typeof loader === 'string'
                ? loader.includes('source-map-loader')
                : loader && loader.loader && loader.loader.includes('source-map-loader')
            );
            if (hasSourceMapLoader) {
              const excludes = [/(\\|\/)pdfjs-dist(\\|\/)/, /(\\|\/)react-pdf(\\|\/)/];
