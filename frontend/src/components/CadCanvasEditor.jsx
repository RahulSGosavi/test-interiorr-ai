...
const CadCanvasEditor = forwardRef(
  (
    ...
    const beginDraft = useCallback(
      ...
      [activeTool, commitShapes, dash, eraseAtPoint, openTextEditor, shapes, strokeColor, strokeWidth, getPointer]
    );
    ...
    const handleStagePointerDown = useCallback(
      (evt) => {
        ...
        // Handle pan/move tools - pan the canvas
        ...
        // Handle select tool - only select shapes, don't draw
        ...
        // All other tools (rect, circle, line, arrow, free, polygon, eraser) - begin drawing
        beginDraft(stage);
      },
    ...
    const handleStagePointerMove = useCallback(
      (evt) => {
        ...
        // Handle two-finger pinch zoom (priority over other actions)
        ...
        // Handle panning (move/pan tools)
        if (isPanning && (activeTool === "pan" || activeTool === "move")) {
          const pointer = stage.getPointerPosition();
        ...
        // Handle other drawing tools (rect, circle, line, arrow, free, polygon)
        if (draft && activeTool !== "pan" && activeTool !== "move" && activeTool !== "select") {
          updateDraft(stage);
        }
      },
      ...
      () =>
        shapes.map((shape) => {
          ...
          const common = {
            ...
            strokeWidth: shape.strokeWidth,
            dash: shape.dash,
            listening: !disabled && activeTool !== "pan" && activeTool !== "move",
            draggable: !disabled && activeTool === "select",
    ...
    return (
      <div ref={containerRef} className="relative h-full w-full overflow-hidden bg-slate-950">
        <Stage
          ...
          // Improve touch responsiveness for tablets/iPads
